...
namespace IMS
{
    public partial class OFundDebitM : System.Web.UI.Page
    {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Date_Load = DAL.OFundMode.Insert_OFundDebitM(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["PID"]), DDL_Style.SelectedValue.ToString(), TB_Subject.Text, TB_Date.Text, Convert.ToDecimal(TB_Price.Text), Convert.ToDecimal(TB_Tax.Text), Convert.ToDecimal(TB_SumPrice.Text), Session["Name"].ToString(), TB_Docu.Text, "D");
                    SqlDataSource1.ConnectionString = Utility.DBconnection.connect_string(Session["DatabaseName"].ToString());
                    SqlDataSource1.SelectCommand = "Select * From OFund_Modify LEFT OUTER JOIN OFund_Main ON OFund_Modify.BelongOFID = OFund_Main.OFID WHERE OFund_Modify.PID = " + Session["PID"] + " And OFund_Modify. IncreaseDebit='D'";                   
