...
namespace IMS
{
    public partial class KCheckListWItemEdit : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            String ID = Session["KCheckListId"].ToString();

            SqlConnection connStr = new SqlConnection(Utility.DBconnection.connect_string(Name));
            SqlDataReader reader;   //宣告一個DataReader

            connStr.Open();
            String select1 = "select * from KCheckList where  KCheckListId='" + ID + "'";  //宣告SQL語法的字串，這邊可依照自行需求修改
            ...
            {
                String KCheckListName = reader.GetString(3);
                Label6.Text = KCheckListName;
            }
            reader.Close();
            connStr.Close();
            ...
            SqlAll.SelectCommand = "select * from KCheckListItem where  KCheckListId='" + ID + "' and KCheckListItemPhase='施工前'";

            SqlDataSource1.ConnectionString = Utility.DBconnection.connect_string(Session["DatabaseName"].ToString());
            SqlDataSource1.SelectCommand = "select * from KCheckListItem where  KCheckListId='" + ID + "' and KCheckListItemPhase='施工中'";

            SqlDataSource2.ConnectionString = Utility.DBconnection.connect_string(Session["DatabaseName"].ToString());
            SqlDataSource2.SelectCommand = "select * from KCheckListItem where  KCheckListId='" + ID + "' and KCheckListItemPhase='施工後'";
        }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete")
            {
                ...
                SqlAll.DeleteCommand = "delete from [KCheckListItem] where [KCheckListItemId] = @KCheckListItemId";
            ...
            if (e.CommandName == "Update")
            {
                ...
                SqlAll.UpdateCommand = "UPDATE [KCheckListItem] SET [KCheckListItemName] = @KCheckListItemName  WHERE [KCheckListItemId] = @KCheckListItemId";
            }
        ...
        protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                //SqlDataSource1.DeleteCommand = "UPDATE [Bid_PreData] SET [status] = 1 WHERE [UID] = @UID";
                SqlDataSource1.DeleteCommand = "delete from [KCheckListItem] where [KCheckListItemId] = @KCheckListItemId";
                ScriptManager.RegisterStartupScript((System.Web.UI.Page)HttpContext.Current.Handler, this.GetType(), "ShowMessage", "alert(' 已刪除');", true);
                GridView2.DataBind();
            }
            if (e.CommandName == "Update")
            {
                ...
                //TextBox LbForecastBiddingDate = (TextBox)GridView1.Rows[ID_class].FindControl("LbForecastBiddingDate");
                //LbForecastBiddingDate.Text = (LbForecastBiddingDate.Text != "") ? DateTime.Parse(LbForecastBiddingDate.Text).ToString("yyyy-MM-dd") : "";
                SqlDataSource1.UpdateCommand = "UPDATE [KCheckListItem] SET [KCheckListItemName] = @KCheckListItemName  WHERE [KCheckListItemId] = @KCheckListItemId";
            }
        ...
        protected void GridView3_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                //SqlDataSource1.DeleteCommand = "UPDATE [Bid_PreData] SET [status] = 1 WHERE [UID] = @UID";
                SqlDataSource2.DeleteCommand = "delete from [KCheckListItem] where [KCheckListItemId] = @KCheckListItemId";
                ScriptManager.RegisterStartupScript((System.Web.UI.Page)HttpContext.Current.Handler, this.GetType(), "ShowMessage", "alert(' 已刪除');", true);
                GridView3.DataBind();
            }
            if (e.CommandName == "Update")
            {
                ...
                //TextBox LbForecastBiddingDate = (TextBox)GridView1.Rows[ID_class].FindControl("LbForecastBiddingDate");
                //LbForecastBiddingDate.Text = (LbForecastBiddingDate.Text != "") ? DateTime.Parse(LbForecastBiddingDate.Text).ToString("yyyy-MM-dd") : "";
                SqlDataSource2.UpdateCommand = "UPDATE [KCheckListItem] SET [KCheckListItemName] = @KCheckListItemName  WHERE [KCheckListItemId] = @KCheckListItemId";
            }
        ...
        protected void SaveAction(object sender, EventArgs e)
        {
            ...
            //確認資料庫是否有資料
            if (datacheck(NewKCheckListItemName.Text.ToString()) == false)
            {

                String SQLKCheckListItemName = NewKCheckListItemName.Text;

                SqlConnection connStr = new SqlConnection(Utility.DBconnection.connect_string(database));

                connStr.Open(); //開啟資料庫的連結
                String select = "INSERT INTO KCheckListItem(KCheckListItemName,KCheckListItemPhase,KCheckListId) Values('" + SQLKCheckListItemName + "','" + phaseDropDownList.Text + "','" + ID + "')";   //宣告SQL語法的字串，這邊可依照自行需求修改
        ...
        public bool datacheck(string data)
        {
            ...
            String select1 = "select * from KCheckListItem where KCheckListItemName = '" + NewKCheckListItemName.Text + "'";  //宣告SQL語法的字串，這邊可依照自行需求修改
            SqlCommand cmd1 = new SqlCommand(select1, connStr);   //宣告SqlCommand並將SQL語法及連結語法帶入
            reader = cmd1.ExecuteReader();   //使用SqlCommand的ExecuteReader()方法，
                                             //ExecuteReader()為查詢時使用，如要刪除、修改、新增，須改為ExecuteNonQuery()方法
