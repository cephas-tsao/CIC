...
namespace IMS
{
    public partial class OFundIncreaseApproval : System.Web.UI.Page
    {
        ...
        {
            bool Result = DAL.OFundMode.OFund_Search4(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), "I");
            if (Result)
            {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                decimal SUM_SumPrice = DAL.OFundMode.SumPrice_Search(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), "I");
                Result = DAL.OFundMode.Update_OFundDebitAssign_3(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), SUM_SumPrice, "AItem10");
            }
            if (Result)
            {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Date_Load = DAL.OFundMode.Insert_OFundDebitM_2(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["PID"]), DDL_Style.SelectedValue.ToString(), TB_Subject.Text, TB_Date.Text, Convert.ToDecimal(TB_Price.Text), Convert.ToDecimal(TB_Tax.Text), Convert.ToDecimal(TB_SumPrice.Text), Session["Name"].ToString(), TB_Docu.Text, "I", Convert.ToInt32(Session["OFID"]), Convert.ToInt32(Session["OFID"]));
                    decimal SUM_SumPrice_1 = DAL.OFundMode.SumPrice_Search_11(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), "I");
                    Result = DAL.OFundMode.Update_OFundDebitAssign_3(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), SUM_SumPrice_1, "OItem10");
                    decimal SUM_SumPrice_2 = DAL.OFundMode.SumPrice_Search(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), "I");
                    Result = DAL.OFundMode.Update_OFundDebitAssign_3(Session["DatabaseName"].ToString(), Convert.ToInt32(Session["OFID"]), SUM_SumPrice_2, "AItem10");
                    GridView1_GVgetData();
                }
                else
                {
