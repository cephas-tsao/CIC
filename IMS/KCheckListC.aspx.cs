...
namespace IMS
{
    public partial class KCheckListC : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //SqlCheckList.SelectCommand = "SELECT * FROM KCheckListTime  where  KCheckListProject ='" + Projectid + "'";
                SqlCheckList.SelectCommand = "select * from KCheckList INNER JOIN KCheckListTime ON KCheckList.KCheckListId = KCheckListTime.CheckList where KCheckList.KCheckListProject = '" + Projectid + "' ORDER BY DateTime DESC";
            }
        }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "editrecord")
            {
                ...
                string Datetime = ((Label)GridView1.Rows[ID_class].FindControl("datetime")).Text.Trim();
                Session["KCheckListTimeId"] = ID;
                Session["KCheckListDateTime"] = Datetime;
                OpenNewWindow("KCheckListCContent.aspx", "700", "900");
                ...
                //SqlCheckList.UpdateCommand = "UPDATE [KCheckList] SET [Location] = @Location  WHERE [KCheckListId] = @KCheckListId";
            }
            ...
            if (e.CommandName == "Delete")
            {
                ...
                connStrr.Open();
                String selectaa = "delete from [KCheckListEngineeringItemSt] where [CheckList] =@KCheckListId";  //宣告SQL語法的字串，這邊可依照自行需求修改
        ...
        protected void CustomersGridView_RowDataBound(Object sender, GridViewRowEventArgs e)
        {
            ...
            {
                Label LockStatuss = (Label)e.Row.FindControl("LockStatus");
                ImageButton LockBtnn = e.Row.FindControl("LockBtn") as ImageButton;
                
                 Label LockStatustest = (Label)e.Row.FindControl("LockStatustest");
                LockStatustest.Text = LockStatuss.Text;
            }
        }

        public void CreatePDF(string KCheckListTimeId)
        {
            ...
            //查位置跟自主檢查表ID
            String selectt = "select * from KCheckListTime where  KCheckListTimeId = '" + KCheckListTimeId + "'";  //宣告SQL語法的字串，這邊可依照自行需求修改
            ...
            while (reader.Read())   //使用無限迴圈將SQL語法查詢的結果每筆查閱一次
            {
                ...
                Label2.Text = reader.GetString(1); //checklistid
                Label4.Text= reader.GetString(4); //id
                Label3.Text = reader.GetString(5); //位置
                Label5.Text = reader.GetString(2);//日期
                Label6.Text = reader.GetString(3);//人員
            }
            reader.Close();
            //查位置跟自主檢查表ID
            ...
            c = new Chunk("自主檢查表名稱：" + Label4.Text + "\n", ChFont);
            p = new Paragraph(c);
            doc1.Add(p);

           
            c = new Chunk("檢查位置：" + Label3.Text + "\n", ChFont);
            p = new Paragraph(c);
            doc1.Add(p);

            c = new Chunk("檢查日期：" + Label5.Text + "\n", ChFont);
            p = new Paragraph(c);
            doc1.Add(p);

            c = new Chunk("檢查人員：" + Label6.Text + "\n", ChFont);
            p = new Paragraph(c);
            doc1.Add(p);
            ...
            **/
            ...
            String select1 = "select* from KCheckListItem INNER JOIN KCheckListItemSt ON KCheckListItem.KCheckListItemId = KCheckListItemSt.KCheckListItem where DateTime = '" + Label5.Text + "' and CheckListTime = '" + KCheckListTimeId + "' and KCheckListItemPhase = '施工前'";
            ...
            while (reader.Read())   //使用無限迴圈將SQL語法查詢的結果每筆查閱一次
            {
                ...
                Label.Text = KCheckListItemStId.ToString();
            ...
            String select3 = "select* from KCheckListItem INNER JOIN KCheckListItemSt ON KCheckListItem.KCheckListItemId = KCheckListItemSt.KCheckListItem where DateTime = '" + Label5.Text + "' and CheckListTime = '" + KCheckListTimeId + "' and KCheckListItemPhase = '施工中'";  //宣告SQL語法的字串，這邊可依照自行需求修改
            ...
            String select4 = "select* from KCheckListItem INNER JOIN KCheckListItemSt ON KCheckListItem.KCheckListItemId = KCheckListItemSt.KCheckListItem where DateTime = '" + Label5.Text + "' and CheckListTime = '" + KCheckListTimeId + "' and KCheckListItemPhase = '施工後'";  //宣告SQL語法的字串，這邊可依照自行需求修改
            ...
            doc1.Close();
            string fileName = Label1.Text + Label4.Text  + "檢查紀錄.pdf";
